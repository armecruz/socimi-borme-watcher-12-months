...
- Recorre SIEMPRE los últimos 12 meses desde hoy.
- Lee el sumario del BORME (API BOE), Sección II (código C).
- Detecta adopción del régimen SOCIMI en el texto del anuncio (HTML).
- Guarda resultados en SQLite (socimi_borme.db) y exporta CSV (socimi_borme.csv).
"""

from __future__ import annotations
import sys, os, re, time, sqlite3, argparse, datetime as dt
...
HEADERS = {"Accept": "application/json", "User-Agent": "SOCIMI-BORME-Watcher/1.0 (GitHub Actions)"}
DB_FILE = "socimi_borme.db"
CSV_FILE = "socimi_borme.csv"

# Patrones típicos para detectar adopción del régimen SOCIMI
ADOPTION_PATTERNS = [
...
def fetch_sumario(fecha: dt.date) -> Optional[Dict[str, Any]]:
    ...
    if r.status_code == 404:
        return None  # sin sumario (fin de semana/festivo)
    r.raise_for_status()
    return r.json()
...
def cmd_run(_):
    # Ventana de 12 meses hasta hoy (Europe/Madrid)
    ...
    with sqlite3.connect(DB_FILE) as conn:
        ...
        for d in daterange(start, today):
            try:
                ...
                time.sleep(0.4)  # cortesía con el servidor del BOE
...
def build_cli():
    ...
    p_run = sub.add_parser("run", help="Procesa SIEMPRE los últimos 12 meses desde hoy")
